...
describe.each`
  ...
  ${false}
`(
  "we can start a firebase application (query filters: $filters)",
  async ({ filters }) => {
    ...
    vi.resetModules();
    const { mockFirebase, mockInitializeApp } = await import("../mocks/firebase");
    ...
    //       expect(recordDoc).toBeInstanceOf(DocumentReference);
    ...
    //       expect(mockCollection).toHaveBeenCalledWith("cities");
    //       expect(mockWithConverter).toHaveBeenCalledWith(converter);
    //       expect(recordsCol).toBeInstanceOf(CollectionReference);
