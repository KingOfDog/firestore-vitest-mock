...
describe("Queries", () => {
  ...
  test("it can filter firestore comparison queries in subcollections", async () => {
    ...
    expect(scheduleItems.docs[0].ref).toHaveProperty(
      "path",
  ...
  test("it throws an error when comparing to null", () => {
    expect(() =>
      db.collection("animals").where("legCount", ">", null)
    ).toThrow();
    expect(() =>
      db.collection("animals").where("legCount", ">=", null)
    ).toThrow();
    ...
    expect(() =>
      db.collection("animals").where("legCount", "array-contains", null)
    ).toThrow();
    expect(() =>
      db.collection("animals").where("legCount", "array-contains-any", null)
    ).toThrow();
    expect(() =>
      db.collection("animals").where("legCount", "in", null)
    ).toThrow();
    expect(() =>
      db.collection("animals").where("legCount", "not-in", null)
    ).toThrow();
  });
